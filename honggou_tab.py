...
from PyQt6.QtGui import QIcon, QPixmap, QImage, QFont, QColor, QPainter, QPainterPath, QBrush, QPen
...
# ==========================================
def _natural_episode_sort_key(path):
    """Sort tên tập theo số tự nhiên: Tap_2 < Tap_10 < Tap_100.
    Dùng chung cho video gốc và *_dubbed.mp4 để concat luôn đúng thứ tự tập.
    """
...
class HonggouMergeThread(QThread):
    ...
    def _probe_duration_ms(self, ffmpeg, ffprobe, filepath):
        """Đo ĐỘ DÀI LUỒNG VIDEO (ms), không lấy format/container duration.

        Timeline concat và timeline SRT bắt buộc dùng cùng video-duration.
        Nếu lấy format duration của MPEG-TS, AAC priming/audio tail có thể dư
        vài chục ms mỗi tập và dồn thành vài giây ở cuối bộ phim.

        Thứ tự: stream=duration -> quét packet video -> ffmpeg stream-copy null.
        Tuyệt đối không fallback sang format=duration.
        """
        si = None; cf = 0
        if sys.platform == "win32":
            si = subprocess.STARTUPINFO()
        ...
        fps_hint = None
        if ffprobe:
            # 1) Nhanh và chính xác khi container có duration cho video stream.
            try:
                r = subprocess.run(
                    [ffprobe, '-v', 'error', '-select_streams', 'v:0',
                     '-show_entries', 'stream=duration,avg_frame_rate', '-of', 'json', filepath],
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                    startupinfo=si, creationflags=cf)
                if r.returncode == 0 and r.stdout:
                    ...
                    if streams:
                        ...
                        try:
                            if dur_s not in (None, '', 'N/A') and float(dur_s) > 0:
                                return int(round(float(dur_s) * 1000))
                        except Exception:
                            pass
                        ...
                        if '/' in afr:
                            try:
                                n, d = afr.split('/', 1)
                                if float(d) != 0:
                                    fps_hint = float(n) / float(d)
                            except Exception:
                                fps_hint = None
            except Exception:
                pass

            # 2) TS/container lạ: quét packet VIDEO, không decode hình.
            try:
                p = subprocess.Popen(
                    [ffprobe, '-v', 'error', '-select_streams', 'v:0',
                     ...
                     '-of', 'csv=p=0', filepath],
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                    startupinfo=si, creationflags=cf, text=True, errors='ignore')
                first_pts = None
                max_pts = None
                dur_at_max = None
                last_good_dur = None
                if p.stdout is not None:
                    for line in p.stdout:
                        ...
                        if len(vals) > 1:
                            try:
                                if vals[1] and vals[1].upper() != 'N/A':
                                    pdur = float(vals[1])
                            except Exception:
                                pdur = None
                        ...
                        if pdur and pdur > 0:
                            last_good_dur = pdur
                p.wait()
                if first_pts is not None and max_pts is not None and max_pts >= first_pts:
                    tail = dur_at_max or last_good_dur
                    if not tail and fps_hint and fps_hint > 0:
                        tail = 1.0 / fps_hint
                    span = (max_pts - first_pts) + (tail or 0.0)
                    if span > 0:
                        return int(round(span * 1000))
            except Exception:
                pass

        # 3) Máy thiếu ffprobe: packet-copy video sang null và đọc out_time.
        if ffmpeg:
            try:
                r = subprocess.run(
                    [ffmpeg, '-v', 'error', '-fflags', '+genpts',
                     '-progress', 'pipe:1', '-nostats', '-i', filepath,
                     '-map', '0:v:0', '-an', '-c:v', 'copy', '-f', 'null', '-'],
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                    startupinfo=si, creationflags=cf)
                progress = r.stdout.decode('utf-8', errors='ignore')
                out_time = None
                for line in progress.splitlines():
                    if line.startswith('out_time='):
                        out_time = line.split('=', 1)[1].strip()
                if out_time and out_time != 'N/A':
                    m = re.match(r'(\d+):(\d+):(\d+(?:\.\d+)?)', out_time)
                    if m:
                        h, mn, sec = m.groups()
                        total_s = int(h)*3600 + int(mn)*60 + float(sec)
                        if total_s > 0:
    ...
    def _detect_gpu_encoder(self, ffmpeg, family):
        """Dò encoder GPU THẬT SỰ chạy được, không chỉ nhìn tên trong `ffmpeg -encoders`.

        Bản cũ dễ chọn nhầm NVENC chỉ vì FFmpeg có build encoder này, dù máy không
        có NVIDIA. Khi NVENC fail nó rớt thẳng CPU và bỏ qua QSV/AMF -> re-encode
        mỗi tập cực chậm. Bản này test encode 1 frame rất nhỏ và cache kết quả.
        """
        ...
        # Chỉ test những encoder có trong build FFmpeg trước.
        enc_txt = ""
        try:
            r = subprocess.run([ffmpeg, "-hide_banner", "-encoders"],
                               stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                               startupinfo=si, creationflags=cf, timeout=10)
        ...
        found = ""
        for enc in order:
            if enc_txt and enc not in enc_txt:
                continue
            try:
                # Test 1 frame 64x64: rất nhanh nhưng đủ biết driver/GPU có dùng được không.
                ...
                rr = subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE,
                                    startupinfo=si, creationflags=cf, timeout=12)
                if rr.returncode == 0:
                    found = enc
                    break
            except Exception:
    ...
    def _enc_args_video(self, ffmpeg, use_gpu, family, gop):
        ...
        if use_gpu:
            ...
            if enc and "nvenc" in enc:
                return ["-c:v", enc, "-preset", "p1", "-cq", "23",
            ...
            if enc and "amf" in enc:
                return ["-c:v", enc, "-quality", "speed", "-qp_i", "23",
                        "-qp_p", "23", "-g", g, "-pix_fmt", "yuv420p"] \
                       + (["-tag:v", "hvc1"] if family == "hevc" else [])
            if enc and "videotoolbox" in enc:
                return ["-c:v", enc, "-q:v", "55", "-g", g, "-pix_fmt", "yuv420p"] \
                       + (["-tag:v", "hvc1"] if family == "hevc" else [])
        # CPU fallback. Khi merge đang chạy 4 tập song song, KHÔNG để mỗi
        # tiến trình x264/x265 tự ăn toàn bộ CPU (4 x all-cores sẽ nghẽn ngược).
        # Chia số thread CPU tương đối đều theo số worker merge hiện tại.
        _mw = max(1, int(getattr(self, "_merge_parallel_workers", 1) or 1))
        _cpu_threads = max(1, int((os.cpu_count() or 4) / _mw))
        if family == "hevc":
            return ["-c:v", "libx265", "-preset", "ultrafast", "-crf", "24",
    ...
    def _fix_episode_to_ts(self, ffmpeg, fp, ts_path, tw, th, tfps, has_audio, _run,
        ...
        def _build(video_args):
            ...
            if has_audio:
                ...
                cmd += ['-an']
            cmd += ['-muxdelay', '0', '-muxpreload', '0', '-f', 'mpegts', ts_path]
            return cmd
    ...
    def _remux_episode_to_ts(self, ffmpeg, fp, ts_path, vcodec, has_audio, _run):
        ...
        if has_audio:
            cmd = [ffmpeg, '-y', '-fflags', '+genpts', '-i', fp,
                   ...
                   '-muxdelay', '0', '-muxpreload', '0', '-f', 'mpegts', ts_path]
        else:
            cmd = [ffmpeg, '-y', '-fflags', '+genpts', '-i', fp,
                   '-c:v', 'copy', '-bsf:v', bsf, '-an', '-muxdelay', '0', '-muxpreload', '0', '-f', 'mpegts', ts_path]
        res = _run(cmd)
        ...
        if has_audio:
            ...
            cmd2 = [ffmpeg, '-y', '-fflags', '+genpts', '-i', fp,
                    '-c:v', 'copy', '-an', '-muxdelay', '0', '-muxpreload', '0', '-f', 'mpegts', ts_path]
        res = _run(cmd2)
    ...
    def run(self):
        ...
        for i, task in enumerate(self.merge_tasks):
            ...
            try:
                ...
                # ── 3) CHUẨN HÓA ĐA LUỒNG -> .ts ────────────────────────
                # Đã quét HẾT metadata ở bước 1/2 rồi mới xử lý. Các tập độc lập
                # nên có thể remux / re-encode song song mà KHÔNG ảnh hưởng thứ tự
                # concat: kết quả được đặt lại vào ts_files đúng index gốc.
                #
                # 4 worker là mức cân bằng tốt: tận dụng NVENC/QSV/AMF nhưng không
                # dồn I/O/VRAM quá mạnh như 8-10 tiến trình FFmpeg cùng lúc.
                from concurrent.futures import ThreadPoolExecutor, as_completed

                PARALLEL_FIX_WORKERS = 4
                worker_count = max(1, min(PARALLEL_FIX_WORKERS, len(probes)))
                self._merge_parallel_workers = worker_count

                # Warm-up GPU encoder 1 lần TRƯỚC khi mở pool để tránh 4 thread
                # cùng lúc chạy bài test encoder và tranh ghi _gpu_enc_cache.
                gpu_encoder = self._detect_gpu_encoder(ffmpeg_path, target_family)
                _gpu_label = gpu_encoder if gpu_encoder else "CPU fallback"
                self.progress_msg.emit(
                    f"🚀 Chuẩn hoá đa luồng: {worker_count} tập/lần · {_gpu_label} · "
                    f"{mismatched} tập cần re-encode — Phần {i+1}/{total_tasks}")

                ts_files = [None] * len(probes)

                def _normalize_one(j, p):
                    fp = p["fp"]
                    ts_path = fp + ".ts"
                    res_off = (tw and th and p["w"] and p["h"]
                               and (p["w"], p["h"]) != (tw, th))
                    ...
                    try:
                        if res_off or fps_off:
                            kind = "fix"
                            detail = (("res" if res_off else "")
                                      + ("+fps" if (res_off and fps_off)
                                         else ("fps" if fps_off else "")))
                            self.progress_msg.emit(
                                f"🛠 [{j+1}/{len(probes)}] Bắt đầu sửa {detail}...")
                            ...
                            self.progress_msg.emit(
                                f"⚡ [{j+1}/{len(probes)}] Bắt đầu remux nhanh...")
                            ok = self._remux_episode_to_ts(
                                ffmpeg_path, fp, ts_path, p["vcodec"], p["has_audio"], _run)
                        return j, ts_path, bool(ok), kind, detail, ""
                    except Exception as ex:
                        return j, ts_path, False, kind, detail, str(ex)

                done_count = 0
                with ThreadPoolExecutor(max_workers=worker_count,
                                        thread_name_prefix="merge_fix") as ex:
                    futs = {ex.submit(_normalize_one, j, p): j
                            for j, p in enumerate(probes)}
                    for fut in as_completed(futs):
                        j, ts_path, ok, kind, detail, err = fut.result()
                        if not ok:
                            # Hủy các task chưa chạy; FFmpeg đang chạy sẽ tự kết thúc.
                            for other in futs:
                                other.cancel()
                            _name = os.path.basename(probes[j]["fp"])
                            raise Exception(
                                f"Khong xu ly duoc tap {j+1} ({_name})"
                                + (f": {err[:160]}" if err else ""))
                        ts_files[j] = ts_path
                        done_count += 1
                        if kind == "fix":
                            self.progress_msg.emit(
                                f"✅ [{done_count}/{len(probes)}] Xong tập {j+1} (đã sửa {detail})")
                        else:
                            self.progress_msg.emit(
                                f"✅ [{done_count}/{len(probes)}] Xong tập {j+1} (remux)")

                # type guard: nếu vì lý do bất thường thiếu 1 slot thì dừng, không
                # concat thiếu tập rồi tạo file trọn bộ sai.
                if any(not x for x in ts_files):
                    raise Exception("Thieu file TS sau buoc chuan hoa da luong")

                # ── 3b) MASTER TIMELINE: VIDEO duration của từng .ts ─────────
                # Cùng một dãy duration dùng cho BOTH:
                #   1) ffconcat đặt mốc bắt đầu tập kế tiếp
                #   2) durations.json để cộng offset SRT.
                # Không dùng format duration của TS vì AAC thường dư ~20ms/tập.
                durations_map = {}
                ...
                for p, ts_path in zip(probes, ts_files):
                    ...
                    if not dms or dms <= 0:
                        raise Exception(
                            f"Khong doc duoc VIDEO duration cua {os.path.basename(p['fp'])}. "
                            "Can ffprobe.exe canh ffmpeg.exe de gop timeline chinh xac.")
                    dms = int(dms)
                    durations_map[os.path.basename(p["fp"])] = dms
                    episode_durations_ms.append(dms)

                dur_json = os.path.join(
                    self.movie_folder, os.path.splitext(out_name)[0] + ".durations.json")
                with open(dur_json, "w", encoding="utf-8") as _df:
                    json.dump(durations_map, _df, ensure_ascii=False, indent=2)

                temp_files = list(ts_files)

                # ── 4) GHÉP: ffconcat dùng CHÍNH duration của video stream ─────
                with open(list_txt_path, 'w', encoding='utf-8') as f:
                    f.write("ffconcat version 1.0\n")
                    for tp, dms in zip(ts_files, episode_durations_ms):
                        _p = tp.replace(os.sep, '/').replace("'", "'\\''")
                        f.write(f"file '{_p}'\n")
                        f.write(f"duration {dms / 1000.0:.6f}\n")
                temp_files.append(list_txt_path)

                self.progress_msg.emit(f"⚡ Ráp mạch phim phần {i+1}/{total_tasks}: {out_name}...")
                # Video vẫn stream-copy (nhanh, không giảm chất lượng). Audio AAC
                # được encode lại 1 lần để xử lý sạch packet audio dư ở ranh giới
                # khi ffconcat dùng video-duration; tránh Non-monotonic DTS/glitch.
                if any_have_audio:
                    ...
                    cmd = [ffmpeg_path, '-y', '-f', 'concat', '-safe', '0', '-i', list_txt_path,
                           '-c:v', 'copy', '-an', '-movflags', '+faststart', final_output]
                result = _run(cmd)
                ...
                if result.returncode == 0 and os.path.exists(final_output):
                    ...
                    if ok_verify:
                        expected_ms = sum(episode_durations_ms)
                        actual_ms = self._probe_duration_ms(ffmpeg_path, ffprobe_path, final_output)
                        if actual_ms:
                            drift_ms = int(actual_ms) - int(expected_ms)
                            self.progress_msg.emit(
                                f"🧭 Timeline check: dự kiến {expected_ms/1000:.3f}s · "
                                f"video {actual_ms/1000:.3f}s · lệch {drift_ms:+d}ms")
                            # Chỉ chặn khi sai lớn; vài frame cuối là bình thường.
                            if abs(drift_ms) > max(1000, len(episode_durations_ms) * 20):
                                raise Exception(
                                    f"Timeline output lech {drift_ms}ms so voi manifest; "
                                    "dung de tranh tao SRT tron bo sai.")
                        success = True
                    else:
...
def _upsize_duju_cover(url):
    ...
    if not url:
        return url
    try:
        # dạng shrink:128w / shrink:192w (chỉ rộng) -> shrink:360:0
        url = re.sub(r'shrink:\d+w', 'shrink:360:0', url)
        # dạng shrink:128:0 (rộng nhỏ) -> shrink:360:0 nếu số < 300
        def _bump(m):
            w = int(m.group(1))
            return f'shrink:{max(w,360)}:0' if w < 300 else m.group(0)
...
class DujuRankThread(QThread):
    ...
    Mỗi item.series: slug(=series_id chung hongguo), title_vi, cover,
    episode_cnt, video_tag_info_vi. item: score, play_cnt.
    Bấm phim -> ghép series_id sang hongguo để quét & tải."""
    item_loaded_signal = pyqtSignal(dict)
    finished_signal = pyqtSignal()

    def __init__(self, genre_tag, max_movies=40, sub_id=None):
        super().__init__()
        self.genre_tag = genre_tag
        self.max_movies = max_movies
        # sub_id truyền thẳng từ UI (loại + BXH). Nếu không có, suy từ genre_tag
        # cũ (tương thích ngược cho các nút chưa đổi).
        self.sub_id = sub_id or {
            "DUJU_prestige": "ranklist_hot_sc",
            "DUJU_hot":      "ranklist_prestige",
            "DUJU_play":     "ranklist_hot_play_sc",
            "DUJU_search":   "ranklist_hot_search_sc",
            "DUJU_new":      "ranklist_new_rank_sc",
            "DUJU_must":     "ranklist_must_watch",
            "DUJU_followed": "ranklist_followed",
        }.get(genre_tag)

    @staticmethod
    def _fmt_count(n):
        """1402334 -> '1.4M', 92000 -> '92k'."""
        try:
            n = int(n)
        except Exception:
            return ""
    ...
    def run(self):
        ...
        try:
            offset, limit = 0, 30
            while len(seen) < self.max_movies:
                url = (f"https://duju.info/api/rank?sub_id={sub_id}"
                ...
                for it in items:
                    ...
                    sid = str(series.get("slug") or it.get("item_id") or "")
                    if not sid or sid in seen:
                        continue
                    seen.add(sid)
                    tags = series.get("video_tag_info_vi") or ""
                    tag_list = [t.strip() for t in re.split(r'[,·]', tags) if t.strip()][:3]
                    self.item_loaded_signal.emit({
                        "title": (series.get("title_vi") or series.get("title_zh")
...
class DujuDetailThread(QThread):
    """Lấy thông tin chi tiết 1 phim từ duju.info/series/<id> (SSR trong HTML).
    Ngày phát hành: aria-label="Lên sóng ngày DD/MM/YYYY".
    Mô tả: <meta name="description">. Diễn viên: actorName/actorRole.
    Thể loại: các tag. Trả 1 dict info qua signal."""
    ...
    def run(self):
        info = {"series_id": self.series_id}
        try:
            headers = {"User-Agent": ("Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                       "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36")}
            r = requests.get(f"https://duju.info/series/{self.series_id}",
                             headers=headers, timeout=15)
            if r.status_code == 200:
                t = r.text
                # Tên phim (title)
                mt = re.search(r'<title>([^<|]+)', t)
                if mt: info["title"] = mt.group(1).strip()
                # Ngày phát hành (aria-label "Lên sóng ngày ...")
                md = re.search(r'aria-label="[^"]*ngày\s*([\d/\-]+)"', t)
                if not md:
                    md = re.search(r'(\d{2}/\d{2}/\d{4})', t)
                if md: info["release_date"] = md.group(1)
                # Điểm số + độ hot (nếu có)
                msc = re.search(r'"score"\s*:\s*"?([\d.]+)', t) or re.search(r'>([\d.]+)</[^>]*>\s*(?:điểm|<)', t)
                # Mô tả (meta description)
                mdesc = re.search(r'<meta name="description" content="([^"]+)"', t)
                if mdesc: info["intro"] = mdesc.group(1).strip()
                # Thể loại: các tag trong detail
                tags = re.findall(r'(?:tag|genre|category)[^"]*"[^>]*>([^<]{2,20})<', t, re.I)
                # dự phòng: lấy các chip thể loại phổ biến
                if not tags:
                    tags = re.findall(r'chip[^>]*>([^<]{2,18})</', t)
                if tags:
                    info["tags"] = [x.strip() for x in tags[:6] if x.strip()]
                # Diễn viên: actorName + actorRole theo cặp
                actors = []
                for mm in re.finditer(
                    r'actorName[^>]*>([^<]+)</div>[\s\S]{0,120}?actorRole[^>]*>([^<]+)</div>', t):
                    actors.append((mm.group(1).strip(), mm.group(2).strip()))
                if actors:
    ...
    def _fetch_one(self, job):
        ...
        if url:
            ...
            if cu.startswith("http"):
                try:
                    rs = requests.get(cu, timeout=8, headers={
                        "User-Agent": ("Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                                       "AppleWebKit/537.36 (KHTML, like Gecko) "
                                       "Chrome/125.0.0.0 Safari/537.36"),
...
class SearchMoviesThread(QThread):
    ...
    def _search_duju_vietnamese(self, keyword, timeout=15):
        """Tìm phim trên duju.info bằng tên TIẾNG VIỆT (SSR trong HTML).
        URL: duju.info/search?q=<tên>. Mỗi phim: <a class="search-module_..._card"
        href="/series/<id>">, img=poster, cardTitle=tên Việt.
        series_id chung hongguo -> tải được. Trả list dict đúng format app."""
        from urllib.parse import quote
        url = f"https://duju.info/search?q={quote(keyword)}"
        headers = {"User-Agent": ("Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                   "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36")}
        ...
        results = []
        seen = set()
        # Tìm TẤT CẢ vị trí "/series/<id>" rồi lấy 1 CỬA SỔ text quanh nó
        # (không dựa vào </a> vì thẻ <a> có thể lồng/đóng sớm -> mất cardTitle).
        for m in re.finditer(r'/series/(\d+)', html):
            sid = m.group(1)
            if sid in seen:
                continue
            seen.add(sid)
            # cửa sổ ~1500 ký tự quanh vị trí để gom đủ img + cardTitle + tags
            win = html[m.start(): m.start() + 1500]

            # Title theo cấu trúc THẬT của duju:
            #  1) <p class="search-module__<hash>_cardTitle">Tên đầy đủ</p>
            #  2) alt của posterImg: <img alt="Tên" class="...posterImg">
            #     (KHÔNG lấy alt="" của posterNen / aria-hidden)
            mt = re.search(
                r'class="[^"]*cardTitle[^"]*"[^>]*>\s*([^<]+?)\s*<', win)
            if not mt:
                # alt của đúng thẻ posterImg (bỏ posterNen alt rỗng)
                mt = re.search(
                    r'alt="([^"]+)"[^>]*class="[^"]*posterImg[^"]*"', win)
            if not mt:
                # dự phòng: alt="..." bất kỳ nhưng phải có nội dung
                mt = re.search(r'alt="([^"]+)"', win)
            title = (mt.group(1).strip() if mt else "")
            if not title or len(title) <= 1:
                title = "Phim"

            # Poster: ưu tiên src của posterImg; nếu không có thì img src đầu tiên
            mi = (re.search(r'class="[^"]*posterImg[^"]*"[^>]*\bsrc="([^"]+)"', win)
                  or re.search(r'\bsrc="([^"]+)"[^>]*class="[^"]*posterImg[^"]*"', win)
                  or re.search(r'<img\b[^>]*\bsrc="(https?://[^"]+)"', win))
            poster = _upsize_duju_cover(mi.group(1)) if mi else ""

            # Số tập: dò nếu duju có nhúng (episode_cnt / "N tập" / "Tập N")
            eps = 0
            me = (re.search(r'"episode_cnt"\s*:\s*"?(\d+)"?', win)
                  or re.search(r'(\d+)\s*[Tt]ập', win)
                  or re.search(r'[Tt]ập\s*(\d+)', win))
            if me:
                try:
                    eps = int(me.group(1))
                except Exception:
    ...
    def run(self):
        ...
        # 3) Server lỗi HOẶC rỗng -> tìm trên DUJU (tên Việt) + HONGGUO (tên Trung),
        #    gộp lại, lọc trùng theo series_id. Gõ Việt hay Trung đều ra.
        combined = []
        seen_sid = set()
        by_sid = {}          # sid -> dict item (để bổ sung số tập chéo nguồn)
        # 3a) duju - tên tiếng Việt
        try:
            for it in self._search_duju_vietnamese(self.keyword):
                sid = str(it.get("series_id"))
                if sid and sid not in seen_sid:
                    seen_sid.add(sid); combined.append(it); by_sid[sid] = it
        except Exception:
            pass
        # 3b) hongguo - tên tiếng Trung. Cùng series_id -> LẤY SỐ TẬP bù cho duju
        #     (duju hay thiếu số tập). Khác series_id -> thêm mới.
        try:
            for it in self._search_web_hongguo(self.keyword):
                sid = str(it.get("series_id"))
                if not sid:
                    continue
                if sid in seen_sid:
                    # đã có (từ duju): bù số tập nếu duju đang 0
                    prev = by_sid.get(sid)
                    if prev and not prev.get("total_episodes"):
                        prev["total_episodes"] = it.get("total_episodes", 0)
                else:
                    seen_sid.add(sid); combined.append(it); by_sid[sid] = it
        except Exception:
            pass
        ...
        if combined:
            self.results_signal.emit(combined)
            return
...
def _hg_find_detail(data, expected_id):
    """Only accept named series matching the requested ID, never recommendations."""
    stack = [data]
    while stack:
        item = stack.pop()
        if isinstance(item, dict):
            sid = item.get('series_id')
            if (isinstance(sid, str) and sid == expected_id
                    and isinstance(item.get('series_name'), str)
                    and item['series_name'].strip()):
                return item
...
def _hg_parse_html(html, expected_id):
    decoder = json.JSONDecoder()
    for match in re.finditer(r'(?:window\s*\.\s*)?_ROUTER_DATA\s*=\s*', html):
        try:
            data, _ = decoder.raw_decode(html[match.end():].lstrip())
            detail = _hg_find_detail(data, expected_id)
            if detail:
                return detail
...
def _hg_browser_detail(url, expected_id):
    try:
        from playwright.sync_api import sync_playwright
    except ImportError as exc:
        raise RuntimeError('Thiếu Playwright trong Python chạy ứng dụng. '
                           'Cài bằng: python -m pip install playwright') from exc
    found = []
    with sync_playwright() as pw:
        ...
        for channel in ('chrome', 'msedge', None):
            try:
                ...
                continue
        if browser is None:
            raise RuntimeError('Không khởi động được Chrome/Edge. Cài Chrome hoặc chạy: '
        ...
        try:
            ...
            def collect(response):
                ...
                if not (host == 'hongguoduanju.com' or host.endswith('.hongguoduanju.com')):
                    return
                if response.request.resource_type not in ('xhr', 'fetch'):
                    return
                try:
                    detail = _hg_find_detail(response.json(), expected_id)
                    if detail:
                        found.append(detail)
                except Exception:
                    pass
            ...
            try:
                ...
                # A slow optional resource must not discard already received data.
                pass
            while time.monotonic() < deadline:
                if found:
                    return found[0]
                try:
                    detail = _hg_parse_html(page.content(), expected_id)
                    if detail:
                        return detail
                    # Serialize in the page to preserve string IDs and avoid handles.
                    raw = page.evaluate('() => JSON.stringify(window._ROUTER_DATA || null)')
                    detail = _hg_find_detail(json.loads(raw), expected_id)
                    if detail:
                        return detail
                    ...
                    if page.is_closed():
                        break
            raise RuntimeError('Trang đã mở nhưng chưa trả dữ liệu phim sau 60 giây. '
                               'Hãy mở link trong Chrome kiểm tra đăng nhập/xác minh '
                               'hoặc phim còn khả dụng, rồi thử lại. Không tạo lượt tải khi thiếu dữ liệu.')
        finally:
            browser.close()


def _hg_episode_count(detail):
    info = detail.get('series_episode_info') or {}
    # Released count takes precedence over planned total for ongoing series.
    for value in (info.get('episode_cnt'), info.get('episode_total_cnt'), detail.get('episode_cnt')):
        try:
            count = int(value)
            if count > 0:
                return count
            ...
            pass
    match = re.search(r'(\d+)', str(detail.get('episode_right_text') or ''))
    if match:
        return int(match.group(1))
    videos = detail.get('vid_list')
...
class HonggouScanThread(QThread):
    ...
    def run(self):
        try:
            ...
            expected_id = parse_qs(parsed.query).get("series_id", [""])[0]
            if not expected_id:
                match = re.search(r"/(?:detail|player)/(\d+)", parsed.path)
                expected_id = match.group(1) if match else ""
            if not expected_id.isdigit():
                self.error_signal.emit("Link chưa có mã phim hợp lệ (series_id). Hãy sao chép link trang chi tiết phim.")
                return
            detail = _hg_parse_html(html, expected_id)
            if not detail:
                try:
                    loader_response = requests.get(
                        "https://hongguoduanju.com/detail",
                        params={"series_id": expected_id, "__loader": "detail_page", "__ssrDirect": "true"},
                        headers={"User-Agent": "Mozilla/5.0", "Referer": self.url}, timeout=25)
                    loader_response.raise_for_status()
                    detail = _hg_find_detail(loader_response.json(), expected_id)
                except (requests.exceptions.RequestException, ValueError):
                    pass
            if not detail:
                try:
                    detail = _hg_browser_detail(self.url, expected_id)
                except Exception as exc:
                    self.error_signal.emit(f"Không đọc được dữ liệu phim: {exc}")
                    return
            ...
            total_episodes = _hg_episode_count(detail)
...
class MovieCard(QFrame):
    ...
    def __init__(self, data, parent=None):
        ...
        self.setStyleSheet("""
            #movieCard { background: transparent; border-radius: 12px; }
            #movieCard:hover { background: #1e293b; }
        """)
        ...
        # --- Poster (bo góc, có overlay badge) ---
        self.poster = QLabel()
        self.poster.setFixedSize(self.CARD_W - 12, self.POSTER_H)
        self.poster.setStyleSheet("background:#0f172a; border-radius:10px;")
        self.poster.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.poster.setText("🎬")
        self._score = str(data.get("score") or "").strip()
        self._play = str(data.get("play_str") or "").strip()
        self._eps = data.get("total_episodes", 0)
        ...
        self.lbl_title.setStyleSheet("color:#f1f5f9; font-size:12px; font-weight:bold;")
        self.lbl_title.setFixedHeight(32)
    ...
    def set_poster(self, img_bytes):
        ...
        pix = QPixmap()
        if not (pix.loadFromData(img_bytes) and not pix.isNull()):
            return
        ...
        out = QPixmap(w, h); out.fill(Qt.GlobalColor.transparent)
        p = QPainter(out)
        ...
        # overlay badge điểm số (góc trên phải)
        if self._score:
            self._draw_badge(p, w-52, 6, 46, 20, f"⭐{self._score}",
                             QColor(0, 0, 0, 160), QColor("#fbbf24"))
        # badge lượt xem (góc trên trái)
        if self._play:
            self._draw_badge(p, 6, 6, 4+len(self._play)*8+16, 20, f"🔥{self._play}",
                             QColor(0, 0, 0, 160), QColor("#f1f5f9"))
        # badge số tập (góc dưới phải)
        if self._eps:
            txt = f"{self._eps} tập"
            self._draw_badge(p, w-4-(len(txt)*7+14), h-26, len(txt)*7+14, 20, txt,
                             QColor(37, 99, 235, 210), QColor("#ffffff"))
        p.end()
        self.poster.setPixmap(out)
        self.poster.setText("")

    def _draw_badge(self, p, x, y, w, h, text, bg, fg):
    ...
    def mousePressEvent(self, e):
        if e.button() == Qt.MouseButton.LeftButton and self.url:
            self.clicked_signal.emit(self.url)
...
class HonggouWidget(QWidget):
    ...
    # Map LOẠI phim -> {BXH: sub_id duju}. Mỗi loại có số BXH khác nhau.
    DUJU_CATEGORIES = [
        ("Phim ngắn", "short"),
        ("Người thật", "human"),
        ("Hoạt hình", "comic"),
    ...
    DUJU_BXH_ORDER = [
        ...
        ("🆕 Phim Mới", "new"),
        ("👀 Đáng Xem", "must"),
        ("❤️ Yêu Thích", "followed"),
    ]
    DUJU_SUBID = {
        "short": {
            "prestige": "ranklist_hot_sc", "hot": "ranklist_prestige",
            "play": "ranklist_hot_play_sc", "search": "ranklist_hot_search_sc",
            "new": "ranklist_new_rank_sc", "must": "ranklist_must_watch",
            "followed": "ranklist_followed",
        },
        "human": {
            "prestige": "human_hot_sc", "play": "human_hot_play",
            "search": "human_hot_search", "new": "human_new_rank",
            "must": "human_must_watch", "followed": "human_followed",
        },
        "comic": {
            "prestige": "comic_series_hot_rank", "play": "comic_series_hot_play",
            "new": "comic_series_new_rank",
        },
        "ai": {
            "prestige": "ai_playlet_hot_sc", "play": "ai_playlet_hot_play",
            "search": "ai_playlet_hot_search", "new": "ai_playlet_new_rank",
            "must": "ai_playlet_must_watch", "followed": "ai_playlet_followed",
        },
    }
    ...
    def _setup_ui(self):
        ...
        # ===== HÀNG 1: các nút hành động (Tất Cả / Sắp Ra Mắt / Đã Lưu / Trọn Bộ)
        self.genre_container = QWidget()
        genre_layout = QHBoxLayout(self.genre_container)
        genre_layout.setContentsMargins(0, 5, 0, 4)
        genre_layout.setSpacing(10)

        self.genre_buttons = []
        actions = [("🔥 Tất Cả", None), ("📅 Sắp Ra Mắt", "DUJU_schedule"), ("💾 Phim Đã Lưu", "SAVED")]
        for name, tag in actions:
        ...
        # ===== HÀNG 2: LOẠI phim (Phim ngắn / Người thật / Hoạt hình / Phim AI)
        self.cat_container = QWidget()
        cat_layout = QHBoxLayout(self.cat_container)
        cat_layout.setContentsMargins(0, 2, 0, 2)
        cat_layout.setSpacing(8)
        self.cat_buttons = []
        self._current_cat = "short"   # loại đang chọn
        for label, key in self.DUJU_CATEGORIES:
            b = QPushButton(label)
            b.setCursor(Qt.CursorShape.PointingHandCursor)
            b.setProperty("cat_key", key)
            b.clicked.connect(lambda ch, k=key: self._on_cat_clicked(k))
            cat_layout.addWidget(b)
            self.cat_buttons.append(b)
        cat_layout.addStretch()
        grid_layout.addWidget(self.cat_container)

        # ===== HÀNG 3: BXH (đổi theo loại đang chọn)
        self.bxh_container = QWidget()
        self.bxh_layout = QHBoxLayout(self.bxh_container)
        self.bxh_layout.setContentsMargins(0, 2, 0, 8)
        self.bxh_layout.setSpacing(8)
        self.bxh_buttons = []
        self._current_bxh = "prestige"   # BXH đang chọn (Đề cử)
        self._active_bxh_mode = False    # chưa vào chế độ BXH khi khởi động
        grid_layout.addWidget(self.bxh_container)

        self._rebuild_bxh_buttons()      # dựng BXH cho loại mặc định
        ...
        # Grid đủ cao để chứa cả card cao nhất (schedule ~336) + đệm -> không đè nhau.
        self.hot_list.setGridSize(QSize(196, 360))
        self.hot_list.setResizeMode(QListWidget.ResizeMode.Adjust)
        self.hot_list.setMovement(QListWidget.Movement.Static)
        self.hot_list.setUniformItemSizes(True)  # mọi ô cùng kích thước -> lưới đều
        ...
        self.hot_list.setSpacing(4)
        self.hot_list.setStyleSheet("QListWidget { background-color: transparent; border: none; outline: none; } QListWidget::item { color: #e2e8f0; border: none; } QListWidget::item:selected { background: transparent; } QScrollBar:vertical { border: none; background: #111827; width: 8px; margin: 0px; } QScrollBar::handle:vertical { background: #374151; border-radius: 4px; min-height: 20px; } QScrollBar::handle:vertical:hover { background: #4b5563; }")
        ...
        self.content_stack.addWidget(self.page_info)
    ...
    def _update_genre_styles(self, active_btn):
        for btn in self.genre_buttons:
            if active_btn is not None and btn == active_btn: btn.setStyleSheet("QPushButton { background-color: #f59e0b; color: #ffffff; font-weight: bold; font-size: 14px; border-radius: 16px; padding: 8px 20px; border: none; } QPushButton:hover { background-color: #d97706; }")
    ...
    def _on_genre_clicked(self, clicked_btn):
        ...
        # Bấm nút hành động -> bỏ chọn hàng loại/BXH (không phải chế độ duju rank).
        self._active_bxh_mode = False
        self._update_cat_styles(active=False)
        self._update_bxh_styles(active=False)
        genre_tag = clicked_btn.property("genre_tag")
        self.url_input.clear()
        self.load_hot_movies_shelf(genre_tag)

    # ---- Hàng LOẠI phim ----
    def _update_cat_styles(self, active=True):
        for b in self.cat_buttons:
            on = active and (b.property("cat_key") == self._current_cat)
            b.setStyleSheet(
                "QPushButton { background-color: %s; color:#fff; font-weight:bold; "
                "font-size:13px; border-radius:14px; padding:6px 16px; border:none; } "
                "QPushButton:hover { background-color:#334155; }"
                % ("#10b981" if on else "#1e293b"))

    def _on_cat_clicked(self, cat_key):
        self._current_cat = cat_key
        # Nếu BXH hiện tại không tồn tại trong loại mới -> về "prestige" (Đề cử).
        if self._current_bxh not in self.DUJU_SUBID.get(cat_key, {}):
            self._current_bxh = "prestige"
        self._rebuild_bxh_buttons()
        self._update_cat_styles(active=True)
        self._load_duju_rank()

    # ---- Hàng BXH (đổi theo loại) ----
    def _rebuild_bxh_buttons(self):
        # xóa nút cũ
        for b in self.bxh_buttons:
            b.setParent(None)
        self.bxh_buttons = []
        avail = self.DUJU_SUBID.get(self._current_cat, {})
        for label, key in self.DUJU_BXH_ORDER:
            if key not in avail:
                continue  # loại này không có BXH đó
            b = QPushButton(label)
            b.setCursor(Qt.CursorShape.PointingHandCursor)
            b.setProperty("bxh_key", key)
            b.clicked.connect(lambda ch, k=key: self._on_bxh_clicked(k))
            self.bxh_layout.insertWidget(self.bxh_layout.count(), b)
            self.bxh_buttons.append(b)
        self._update_bxh_styles(active=getattr(self, '_active_bxh_mode', True))

    def _update_bxh_styles(self, active=True):
        for b in self.bxh_buttons:
            on = active and (b.property("bxh_key") == self._current_bxh)
            b.setStyleSheet(
                "QPushButton { background-color: %s; color:#fff; font-weight:bold; "
                "font-size:12px; border-radius:12px; padding:6px 14px; border:none; } "
                "QPushButton:hover { background-color:#d97706; }"
                % ("#f59e0b" if on else "#0ea5e9"))

    def _on_bxh_clicked(self, bxh_key):
        self._current_bxh = bxh_key
        self._load_duju_rank()

    def _load_duju_rank(self):
        """Tải BXH duju theo loại + BXH đang chọn."""
        self._active_bxh_mode = True
        self.url_input.clear()
        self._update_genre_styles(None)   # bỏ sáng nút hành động
        self._update_cat_styles(active=True)
        self._update_bxh_styles(active=True)
        sub_id = self.DUJU_SUBID.get(self._current_cat, {}).get(self._current_bxh)
        if sub_id:
            self.load_hot_movies_shelf(("DUJU_SUB", sub_id))
    ...
    def load_hot_movies_shelf(self, genre=None):
        ...
        if genre: msg = "⏳ Đang lọc phim, đợi 1 - 2 phút..."
        ...
        # genre dạng tuple ("DUJU_SUB", sub_id) -> BXH duju theo loại+bảng chọn.
        if isinstance(genre, tuple) and genre and genre[0] == "DUJU_SUB":
            self.day_bar.hide()
            self._duju_missing_covers = []
            self._duju_card_by_row = {}
            self.hot_thread = DujuRankThread("DUJU_SUB", max_movies=40, sub_id=genre[1])
        ...
        if isinstance(genre, str) and genre.startswith("DUJU_"):
            ...
            if genre == "DUJU_schedule":
                ...
                self.day_bar.hide()
                self.hot_thread = DujuRankThread(genre, max_movies=40)
        else:
            self.day_bar.hide()
    ...
    def _on_hot_movies_finished(self):
        ...
        # Phim duju: tải ảnh bìa nền rồi vẽ vào card (bo góc + badge).
        duju_covers = getattr(self, '_duju_missing_covers', None)
        if duju_covers:
            self._duju_missing_covers = []
            if not (getattr(self, 'duju_cover_thread', None) and self.duju_cover_thread.isRunning()):
                self.duju_cover_thread = HistoryCoverThread(
                    duju_covers, self._get_covers_dir(), self.auth_token)
                self.duju_cover_thread.cover_ready.connect(self._on_history_cover_ready)
                self._keep_thread_alive(self.duju_cover_thread)
                self.duju_cover_thread.start()

    def _on_history_cover_ready(self, row, img_bytes):
        # Nếu row này là card duju -> vẽ poster bo góc + badge vào card.
        card = getattr(self, '_duju_card_by_row', {}).get(row)
        if card is not None:
            try:
                card.set_poster(img_bytes)
            except Exception:
                pass
            return
        # Ngược lại: item thường (lịch sử / server) -> gán icon.
    ...
    def _render_single_hot_movie(self, m, _direct=False):
        ...
        # _direct=True: gọi thẳng (VD danh sách Phim Đã Lưu), không qua thread.
        if not _direct and self.sender() is not getattr(self, 'hot_thread', None):
            return
        ...
        if m.get("from_duju"):
            ...
            item = QListWidgetItem()
            # sizeHint = gridSize để KHÔNG bị chồng/lệch (card tự đặt fixed size,
            # nhưng ô list phải bằng grid thì lưới mới đều).
            item.setSizeHint(self.hot_list.gridSize())
            item.setData(Qt.ItemDataRole.UserRole, card_url or "")
            self.hot_list.addItem(item)
            self.hot_list.setItemWidget(item, card)
            row = self.hot_list.count() - 1
            self._duju_card_by_row = getattr(self, '_duju_card_by_row', {})
            self._duju_card_by_row[row] = card
            if m.get("cover_url"):
                if not hasattr(self, '_duju_missing_covers'):
                    self._duju_missing_covers = []
                self._duju_missing_covers.append((row, series_id, m.get("cover_url")))
            return

        # Phim server -> giữ nguyên cách cũ (icon + text).
        item = QListWidgetItem()
        downloaded_tag = "\n[✅ Đã tải trong bộ nhớ]" if series_id in self._cached_history_ids else ""
        item.setText(f"{title}\n({eps} Tập){downloaded_tag}")
        item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
        item.setSizeHint(self.hot_list.gridSize())  # đồng nhất ô -> lưới đều
    ...
    def _on_card_clicked(self, url):
        ...
        if not sid:
            return
        # Mở TRANG THÔNG TIN phim (ngày, mô tả, diễn viên) trước, có nút Tải.
        self._open_info_page(sid, url)

    def _open_info_page(self, series_id, url):
        """Hiện trang thông tin phim kiểu duju, fetch detail nền."""
        self._info_url = url
        self._info_sid = str(series_id)
        # Dựng khung loading
        for i in reversed(range(self.info_layout.count())):
            w = self.info_layout.itemAt(i).widget()
            if w: w.setParent(None)
        self.content_stack.setCurrentWidget(self.page_info)
        loading = QLabel("⏳ Đang tải thông tin phim...")
        loading.setStyleSheet("color:#94a3b8; font-size:14px;")
        self.info_layout.addWidget(loading)
        # fetch detail duju nền
        self.detail_thread = DujuDetailThread(series_id)
        self.detail_thread.detail_ready.connect(self._on_detail_ready)
        self._keep_thread_alive(self.detail_thread)
        self.detail_thread.start()

    def _on_detail_ready(self, info):
        if str(info.get("series_id")) != str(getattr(self, '_info_sid', '')):
            return
        # xóa loading
        for i in reversed(range(self.info_layout.count())):
            w = self.info_layout.itemAt(i).widget()
        ...
        btn_back.setStyleSheet("QPushButton { background:#334155; color:#fff; border:none; border-radius:8px; padding:6px 14px; font-weight:bold; } QPushButton:hover { background:#475569; }")
        btn_back.clicked.connect(lambda: self.content_stack.setCurrentWidget(self.page_grid))
        row_top.addWidget(btn_back); row_top.addStretch()
        w_top = QWidget(); w_top.setLayout(row_top)
        self.info_layout.addWidget(w_top)

        # Header: poster + tên + ngày + mô tả
        header = QHBoxLayout(); header.setSpacing(18)
        poster = QLabel(); poster.setFixedSize(180, 240)
        poster.setStyleSheet("background:#0f172a; border-radius:12px;")
        poster.setAlignment(Qt.AlignmentFlag.AlignCenter); poster.setText("🎬")
        # tải poster
        cpath = os.path.join(self._get_covers_dir(), f"{self._info_sid}.img")
        if os.path.exists(cpath):
            pm = QPixmap(cpath)
            if not pm.isNull():
                poster.setPixmap(pm.scaled(180,240,Qt.AspectRatioMode.KeepAspectRatioByExpanding,Qt.TransformationMode.SmoothTransformation))
        header.addWidget(poster)

        right = QVBoxLayout(); right.setSpacing(8)
        lbl_name = QLabel(info.get("title","Phim"))
        lbl_name.setStyleSheet("color:#f1f5f9; font-size:20px; font-weight:bold;")
        lbl_name.setWordWrap(True)
        right.addWidget(lbl_name)
        if info.get("release_date"):
            lbl_date = QLabel(f"📅 Lên sóng ngày: {info['release_date']}")
            lbl_date.setStyleSheet("color:#f59e0b; font-size:14px; font-weight:bold;")
            right.addWidget(lbl_date)
        if info.get("tags"):
            lbl_tags = QLabel("🏷 " + " · ".join(info["tags"]))
            lbl_tags.setStyleSheet("color:#94a3b8; font-size:12px;")
            lbl_tags.setWordWrap(True)
            right.addWidget(lbl_tags)
        # nút tải
        btn_dl = QPushButton("▶️  Tải phim này")
        btn_dl.setCursor(Qt.CursorShape.PointingHandCursor)
        btn_dl.setFixedHeight(42)
        btn_dl.setStyleSheet("QPushButton { background:#059669; color:#fff; font-size:15px; font-weight:bold; border:none; border-radius:10px; padding:8px 24px; } QPushButton:hover { background:#047857; }")
        btn_dl.clicked.connect(self._start_download_from_info)
        right.addWidget(btn_dl)
        right.addStretch()
        w_right = QWidget(); w_right.setLayout(right)
        header.addWidget(w_right, 1)
        w_header = QWidget(); w_header.setLayout(header)
        self.info_layout.addWidget(w_header)

        # Mô tả
        if info.get("intro"):
            lbl_i = QLabel("📖 Nội dung:")
            lbl_i.setStyleSheet("color:#f1f5f9; font-size:15px; font-weight:bold;")
            self.info_layout.addWidget(lbl_i)
            desc = QLabel(info["intro"])
        ...
        # Diễn viên
        if info.get("actors"):
            lbl_a = QLabel("🎭 Diễn viên:")
            lbl_a.setStyleSheet("color:#f1f5f9; font-size:15px; font-weight:bold;")
            self.info_layout.addWidget(lbl_a)
            cast = " · ".join([f"{n} (vai {r})" for n, r in info["actors"]])
            lbl_cast = QLabel(cast)
            lbl_cast.setWordWrap(True)
            lbl_cast.setStyleSheet("color:#cbd5e1; font-size:13px;")
            self.info_layout.addWidget(lbl_cast)

        self.info_layout.addStretch()

    def _start_download_from_info(self):
        """Bấm 'Tải phim này' -> quét hongguo như bình thường."""
        url = getattr(self, '_info_url', '')
        if not url:
            return
        self.url_input.setText(url)
        self._from_shelf = True
        self._scan()

    def _on_hot_movie_clicked(self, item):
        # Nếu item là CARD duju (có widget) -> đã có card.clicked_signal xử lý
        # (mở trang thông tin). Bỏ qua ở đây để không quét thẳng đè lên.
        if self.hot_list.itemWidget(item) is not None:
            return
        url = item.data(Qt.ItemDataRole.UserRole)
        if not url: return
        if getattr(self, 'scan_thread', None) and self.scan_thread.isRunning(): return
    ...
    def _on_scan_result(self, data):
        ...
        # Phim CHƯA LÊN SÓNG: hongguo trả 0 tập (hoặc không có tập nào tải được).
        # Báo rõ thay vì hiện bảng trống "Quét thành công (0 tập)".
        _real_eps = total_eps or len(self.current_episodes)
        if _real_eps <= 0:
            QMessageBox.information(
                self, "Phim chưa lên sóng",
                f"🎬 \"{self.current_title}\" hiện CHƯA có tập nào để tải.\n\n"
                "Phim có thể chưa lên sóng, hoặc nguồn chưa cập nhật. "
                "Bạn đã lưu phim này thì cứ để đó, khi phim ra tập sẽ tải được.\n\n"
                "💡 Mẹo: bấm 💾 Lưu ở mục Sắp Ra Mắt để không bỏ lỡ phim hay!")
            self.lbl_status.setText("⏳ Phim chưa có tập để tải — thử lại sau nhé.")
            self.btn_download.setEnabled(False)
            # quay về danh sách grid thay vì bảng trống
            try:
                self.content_stack.setCurrentWidget(self.page_grid)
            except Exception:
                pass
    ...
    def _do_merge(self, mode, files_to_merge, after_dub=False):
        ...
        # Thread dịch/lồng tiếng có thể hoàn thành không theo thứ tự. Luôn sort
        # theo số tập tự nhiên để Tap_100 không chen trước Tap_11.
        files_to_merge = sorted(
            [f for f in (files_to_merge or []) if os.path.exists(f)],
            key=_natural_episode_sort_key)
        ...
        if mode == 1:
            ...
            for i in range(0, len(files_to_merge), chunk_size):
                ...
                if chunk:
                    # Phần cuối chỉ 1 tập vẫn đi qua cùng MergeThread để MP4 và
                    # SRT được tạo theo một timeline, không chỉ rename mỗi video.
                    merge_tasks.append({
        ...
        for task in merge_tasks:
            ...
            for suffix, srt_suffix in [("_vi.srt", "_vi.srt"), (".srt", ".srt")]:
                ...
                for vf in video_files:
                    ...
                    # Ưu tiên MASTER TIMELINE do MergeThread ghi. Chính các
                    # duration này cũng đã được ghi vào ffconcat, nên video và SRT
                    # dùng đúng cùng một mốc. Không có bảng mới probe riêng v:0.
                    ...
                    if dur:
                        ...
                        # Không đoán duration bằng câu sub cuối + 1 giây. Khoảng
                        # lặng cuối mỗi tập khác nhau -> sai sẽ cộng dồn rất nặng.
                        missing_dur = True
                        break
                ...
                if missing_dur:
                    if hasattr(self, 'txt_stt_log'):
                        self.txt_stt_log.append(
                            f"❌ Không gộp sub {out_name_noext}{suffix}: thiếu VIDEO duration "
                            "của ít nhất 1 tập. Hãy đặt ffprobe.exe cạnh ffmpeg.exe rồi gộp lại."
                        )
                    continue
    ...
    def _merge_after_dub(self):
        ...
        dubbed = sorted(getattr(self, '_gemini_vi_map', {}).keys(), key=_natural_episode_sort_key)
